...
void LivoxMsgCbk1(const livox_ros_driver::CustomMsgConstPtr& livox_msg_in) {
  ...
  for (size_t j = 0; j < livox_data.size(); j++) {
    ...
    for (unsigned int i = 0; i < livox_msg->point_num; ++i) {
      ...
      pt.intensity = livox_msg->points[i].reflectivity; // The integer part is line number and the decimal part is timestamp
      pt.curvature = s*0.1;
      pcl_in.push_back(pt);
    }
  }
...
int main(int argc, char** argv) {
  ...
  pub_pcl_out1 = nh.advertise<sensor_msgs::PointCloud2>("/livox/pointcloud", 100);
